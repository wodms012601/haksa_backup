	...
	<head>
		...
		<script>
			$(document).ready(function() {
				// 조회 버튼 눌렀을 시 페이지 이동
				$("#listOtherAppointment").click(function(){
				...
				$.ajax({
					...
					, success: function(data){
						$(data).each(function(index, item){
							...
							$('#otherAppointmentPositionTeam').append("<option value="+item.teamCode+">"+item.teamNameKorean+"</option>");
						})
					}
				})
	...
	<body id="page-top">
		...
		<div id="wrapper">
			...
			<div id="content-wrapper">
				...
				<h1>인사관리 > 임명관리</h1>
				<br>
				<p id="enrolScoreMain">* 무기계약 임용 정보 입력</p>
				<form id="form">
					<input type="button" class="btn btn-info" id="listOtherAppointment" value="조회"/>
				...
				<table id="table" class="table table-bordered">
					...
					<tr>
						...
						<th>지급연봉</th>
						<td><input type="text" class="form-control" name="otherAppointmentAnnualIncome" id="otherAppointmentAnnualIncome" placeholder="지급연봉"></td>
						<th>경력구분</th>
						<td>
							<select class="form-control" id="otherAppointmentCareerDivision">
								<option value="선택">선택</option>
				  	...
				  	<tr>
				  		<th>호봉</th>
						<td><input type="text" class="form-control" name="otherAppointmentSalaryclass" id="otherAppointmentSalaryclass" placeholder="호봉"></td>
						<th>계약시작기간</th>
						<td><input type="date" class="form-control" name="otherAppointmentContractStartTerm" id="otherAppointmentContractStartTerm"></td>
						<th>계약만료기간</th>
						<td><input type="date" class="form-control" name="otherAppointmentContractEndTerm" id="otherAppointmentContractEndTerm"></td>		
					</tr>
					<tr>
						<th>직위</th>
						<td>
						...
						<th>보(겸)직팀</th>
						<td>
							<select class="form-control" id="otherAppointmentPositionTeam">
								<option value="선택">선택</option>
							</select>
						</td>
						<th>임명일</th>
					...
					<tr>
						<th>호봉승급월</th>
						<td><input type="text" class="form-control" name="otherAppointmentSalaryclassPromotion" id="otherAppointmentSalaryclassPromotion" placeholder="호봉승급월"></td>
						<th>병역여부</th>
						<td>
